...
extension DatabaseManager {
    ...
    public func getDataFor(path: String, completion: @escaping (Result<Any, Error>) -> Void) {
        self.database.child("\(path)").observeSingleEvent(of: .value) { snapshot in
            ...
            completion(.success(value))// pass in child path
        }
    }
    ...
    public func userExists(with email: String,
        ...
        database.child(safeEmail).observeSingleEvent(of: .value, with: { snapshot in
            guard snapshot.value as? String != nil else {
                completion(false)//can't find same email address
                return
            }
            
            completion(true)//can find same email address
        })
    }
    
    /// Insert new users to database
    public func insertUser(with user: ChatAppUser, completion: @escaping (Bool) -> Void){
        database.child(user.safeEmail).setValue([//user's email address need to be unique
            ...
            "last_Name": user.lastName], withCompletionBlock: { error, _ in
                ...
                //get a reference, and append to the new user collection
...
struct ChatAppUser {
    ...
    var profilePictureFileName: String{
        //lipeiwen950112-gmail-com_profile_picture.png
